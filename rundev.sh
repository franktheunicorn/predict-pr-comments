...
./upload_spark_jar.sh 
# Train the model
export APP_PREFIX="ml17a-rf-test"
export MEMORY_OVERHEAD_FRACTION=0.4
...
piddev=$!
MEMORY_OVERHEAD_FRACTION=0.45 VERSION=full APP_NAME="$APP_PREFIX$VERSION" NUM_EXECS=100 SPARK_DEFAULT_PARALLELISM=1000 MAIN_CLASS=com.holdenkarau.predict.pr.comments.sparkProject.ml.MlSCApp INPUT=gs://frank-the-unicorn/$VERSION/output OUTPUT=gs://frank-the-unicorn/$VERSION/ml-$APP_PREFIX CACHE=gs://frank-the-unicorn/$VERSION/ml/dataprep-cache ./run_spark_data_process.sh &
pidfull = $!
wait $pidfull
